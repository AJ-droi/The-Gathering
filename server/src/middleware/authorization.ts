...
export const authPhoto = async(req:JwtPayload, res:Response, next:NextFunction) => {
    try{
        const authorization = req.headers.authorization
        ...
        const user = await PhotographerInstance.findOne({where:{id: id}})as unknown as PhotographerAttributes;
