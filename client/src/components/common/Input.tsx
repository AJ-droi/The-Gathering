...
export const SignInput: React.FC<InputProps> = ({type, placeholder, style, caption}) => {
  ...
  return (
    ...
    <input type={type} placeholder={placeholder} className={`${style} hidden `} />
    <span className='inline border w-[20px] h-[20px] border-[#FF6E31] border-[2px] geekmark'></span>
    <span className='text-[0.6rem] lg:text-[0.9rem]'>{caption}</span>   </label>
    :  type === "password" ? <div className='w-[70%] text-left mx-auto'>
    <label className='text-[#606060]'>{caption}</label>
    <div>
      <input type={showPassword ? 'password' : 'text'} placeholder={placeholder} className={`${style} `} />
